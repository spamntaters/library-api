...
// ToggleRead is the resolver for the toggleRead field.
func (r *mutationResolver) ToggleRead(ctx context.Context, id int) (*Book, error) {
	if err := validatePositiveID("id", id); err != nil {
		return nil, err
	}

	book, err := r.Store.ToggleRead(ctx, int32(id))
	if err != nil {
		return nil, err
	}
	return dbToGraphQLBook(book), nil
